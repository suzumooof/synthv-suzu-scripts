...
function getClientInfo() {
    return {
        name: SV.T('repeat play of current phrase'),
...
function getTranslations(langCode) {
    switch (langCode) {
        case 'ja-jp':
            return [
                ['playback', 'プレイバック'],
                ['repeat play of current phrase', 'フレーズのリピート再生'],
            ];
    }
    return [];
}
...
function main() {
    ...
    if (playback.getStatus() !== 'stopped') {
        ...
        if (nearNotePair === undefined) {
            ...
            // ノートと隣接するフレーズの選択
            var pairList = getPhraseNotePairListFromAllGroups(track, nearNotePair);
            // ブリック単位の選択範囲の取得
