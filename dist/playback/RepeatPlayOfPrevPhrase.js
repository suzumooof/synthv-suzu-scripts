 ...
 * 「前のフレーズのリピート再生」のスクリプトです。
 *
 * 表示中のトラックで現在の再生位置から一番近いフレーズを探して、
 * そこからその前のフレーズ内をリピート再生します。
...
function getClientInfo() {
    return {
        name: SV.T('repeat play of prev phrase'),
...
function getTranslations(langCode) {
    switch (langCode) {
        case 'ja-jp':
            return [
                ['playback', 'プレイバック'],
                ['repeat play of prev phrase', '前のフレーズのリピート再生'],
            ];
    }
    return [];
}
...
function main() {
    ...
    if (pairList.length > 0) {
        nearNotePair = pairList[0];
        nearNotePair = searchNotePairOnTrackByBlick(nearNotePair.onset, track, 'before');
        if (nearNotePair !== undefined) {
            pairList = getPhraseNotePairListFromAllGroups(track, nearNotePair);
        }
        else {
